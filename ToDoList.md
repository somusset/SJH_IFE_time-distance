...
- Think about axis for the time-distance plot: needed? confusing? what is possible within streamlit and canvas?

- Little bugs to fix:
    - Change color map to the original AIA map?
    - Make sure the radio options reset to nothing once a new subject is loaded
    - Implement the "submit and talk" option the same way it works on usual Zooniverse FE
    - Color the submit buttons like it is on Zooniverse
